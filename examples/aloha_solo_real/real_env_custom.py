...
class RealEnv:
    ...
    Action space:      [left_arm_qpos (6),             # absolute joint position
                        left_gripper_positions (1),    # normalized gripper joint angle (0: close, 1: open)
                        right_arm_qpos (6),            # absolute joint position
                        right_gripper_positions (1),]  # normalized gripper joint angle (0: close, 1: open)

    Observation space: {"qpos": Concat[ left_arm_qpos (6),          # absolute joint position
                                        left_gripper_position (1),  # normalized gripper joint angle (0: close, 1: open)
                                        right_arm_qpos (6),         # absolute joint position
                                        right_gripper_qpos (1)]     # normalized gripper joint angle (0: close, 1: open)
    ...
    def get_qpos(self):
        ...
        # NOTE: Using JOINT normalization because data was collected with Interbotix version
        # which uses bot.gripper.get_gripper_position() that returns joint angle (radians).
        # ROS JointState position[6] is the gripper joint angle, position[7] is linear position.
        left_gripper_qpos = [
            constants.PUPPET_GRIPPER_JOINT_NORMALIZE_FN(left_qpos_raw[6])
        ]  # this is joint angle (radians), matching Interbotix API
        right_gripper_qpos = [
