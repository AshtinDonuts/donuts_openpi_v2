...
import openpi.policies.aloha_solo_policy as aloha_solo_policy
...
@dataclasses.dataclass(frozen=True)
class LeRobotAlohaSoloDataConfig(DataConfigFactory):
    # If true, will convert joint dimensions to deltas with respect to the current state before passing to the model.
    # Gripper dimensions will remain in absolute values.
    use_delta_joint_actions: bool = True
    # If provided, will be injected into the input data if the "prompt" key is not present.
    ...
    # Repack transforms for solo arm (single arm with 3 cameras).
    repack_transforms: tyro.conf.Suppress[_transforms.Group] = dataclasses.field(
        default=_transforms.Group(
            inputs=[
                _transforms.RepackTransform(
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        data_transforms = _transforms.Group(
            inputs=[
                aloha_solo_policy.AlohaInputs(
                    adapt_to_pi=self.adapt_to_pi, use_joint_normalization=self.use_joint_normalization
                )
            ],
            outputs=[aloha_solo_policy.AlohaOutputs(adapt_to_pi=self.adapt_to_pi)],
        )
        if self.use_delta_joint_actions:
            # For solo arm: 6 joints (indices 0-5), gripper at index 6
